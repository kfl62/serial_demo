...
class PopulateTables < Sequel::Migration
  ...
  def up
    ...
    DB[:hw_readers].insert(nil,1,"Test reader_1",Time.now(),Time.now())
    DB[:hw_readers].insert(nil,2,"Test reader_2",Time.now(),Time.now())
    DB[:hw_tasks].insert(nil,1,"Open door",Time.now(),Time.now())
    DB[:hw_devices].insert(nil,1,1,"Test device_1",Time.now(),Time.now())
    DB[:hw_devices].insert(nil,2,1,"Test device_1",Time.now(),Time.now())
    DB[:hw_keys].insert(nil,"2900001424C1",Time.now(),Time.now())
    DB[:prs_owners].insert(nil,1,"Attila","Albert",Time.now(),Time.now())
    DB[:prs_admins].insert(nil,1,"csattila","Albert.Attila@evoline.ro",nil,nil,Time.now(),Time.now())
